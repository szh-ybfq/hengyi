...
public class CacheDelayDeleteConsumer {
    ...
    @Resource
    private ProductCacheUtils productCacheUtil;
    ...
    public void consumeDelayedDeleteMsg(CacheDelayMsgDTO msg) {
        ...
        // 场景2：分类分页批量清理缓存
        if ("category_all_type".equals(type) && categoryId != null) {
            // 批量清理该分类下所有类型缓存
            productCacheUtil.clearCategoryAllCache(categoryId);
            log.info("延迟清理该分类下所有类型缓存，categoryId:{}", categoryId);
        }
    }
}
